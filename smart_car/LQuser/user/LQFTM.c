...
void FTM_PWM_Init(FTM_Type * ftmn, FTM_CHn_e ch, u16 mod, u16 cv)
{
  /******************* 开启时钟 和 复用IO口*******************/
   ...
   case FTM3_BASE:
    ...
    /******************** 配置时钟和分频 ********************/
    FTM_SC_REG(ftmn)    = ( 0
                                  | FTM_SC_PS(0)      //分频2^FTM_SC_PS,频率为 275M/2/16
...
void ftm_pwm_duty(FTM_Type * ftmn, FTM_CHn_e ch, uint32 duty)
{
    uint32 cv;
    uint32 mod = 0;
#ifdef DEBUG_MODE
...
void FTM_AB_Init(FTM_Type * ftmn)
{
    ...
    case FTM2_BASE:
          //SIM_SCGC3 |= SIM_SCGC6_FTM2_MASK;                           //使能FTM2时钟
...
void Motor_Init(void)
{
   ...
   FTM_PWM_Init(FTM0,FTM_CH0,10000,  2000);//Mot0-PTC1
   FTM_PWM_Init(FTM0,FTM_CH1,10000,  0);//Mot1-PTC2 
   FTM_PWM_Init(FTM0,FTM_CH2,10000,  0);//Mot2-PTC3
   FTM_PWM_Init(FTM0,FTM_CH3,10000,  2000);//Mot3-PTC4 
}
...
void Test_AB_Pulse_Cnt(void)
{
  ...
  LCD_P8x16Str(13,0,(uint8_t*)"LQ Encoder CNT");
    ...
    ftm_clean(FTM2) ;   
    //if(GPIO_Get(PTA11))             //检测信标板编码器的PTA11是否为高电平,即检测正反转
    //PulsesL = -PulsesL;               //如果反转，脉冲值取反
    PulsesL = PulsesL;
